...
export function makeCharacterCanonical(overrides) {
    const data = {
        id: nextUlid(),
        lineageId: nextUlid(),
        version: 1,
...
export function makeCharacterInstance(overrides) {
    const base = {
        id: nextUlid(),
...
export function makeSceneInstance(overrides) {
    const base = {
        id: nextUlid(),
        kind: 'scene',
        createdAt: new Date().toISOString(),
...
export function validateUlid(candidate) {
