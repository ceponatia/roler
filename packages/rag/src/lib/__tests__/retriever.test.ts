...
const BASE_CONFIG: PgVectorAdapterConfig = {
  kind: 'pgvector',
  connectionString: 'postgres://test:pass@localhost:5432/roler',
  schema: 'public',
  table: 'vector_chunks',
...
describe('createPgVectorRetriever', () => {
  ...
  const retriever = createPgVectorRetriever({ config: BASE_CONFIG, runQuery });
    const res = await retriever.retrieve({ embedding: [0.1, 0.2], k: 10 });

    expect(retriever.kind).toBe('pgvector');
  expect(retriever.config).toBe(BASE_CONFIG);
